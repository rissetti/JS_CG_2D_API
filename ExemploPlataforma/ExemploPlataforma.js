class ExemploPlataforma extends JS_CG_2D_API {
  ...
  mousePressionado(e) {
    // Usa e.x e e.y fornecidos diretamente pela API já escalados para o canvas
    this.alvoMouseX = e.x;

    // Dispara o pulo se o clique ocorrer acima do pé do jogador
    if (e.y < this.jogador.y + this.jogador.h) {
      this.jogador.pular();
    }
  }
  ...
  atualizar() {
    ...
    if (this.teclas["ArrowLeft"] || this.teclas["a"] || this.teclas["A"]) {
      this.jogador.x -= this.velocidadeHorizontal;
      movendoTeclado = true;
    }
    ...
    if (!movendoTeclado && this.alvoMouseX !== null) {
      const centroX = this.jogador.x + this.jogador.w / 2;
      const diffX = this.alvoMouseX - centroX;

      if (Math.abs(diffX) > 6) {
        this.jogador.x += Math.sign(diffX) * this.velocidadeHorizontal;
      } else {
        this.alvoMouseX = null;
      }
    }
  ...
  desenhar() {
    ...
    this.retangulo(this.jogador.x, this.jogador.y, this.jogador.w, this.jogador.h, Estilo.PREENCHIDO);
...
window.addEventListener("load", () => {
  new ExemploPlataforma("Exemplo Jogo de Plataforma", "meuCanvas", 800, 600);
});