class Ponto {
  constructor(x = 0, y = 0) {
    this.x = x;
    this.y = y;
  }
}

class ExemploSnake extends JS_CG_2D_API {
  acaoAoIniciar() {
    EfeitosSonoros.carregarSom("comida", "sons/resgate.wav");
    EfeitosSonoros.carregarSom("tiro", "sons/tiro.mp3");
    EfeitosSonoros.volumeSom("comida", 0.1);

    this.tamanhoQuadrado = 30;
    this.resetarJogo();
  }

  resetarJogo() {
    this.atualizarDimensoes();
    this.tamanhoCobra = 5;
    this.velocidade = new Ponto(0, 0);
    this.posicao = new Ponto(10, 15);
    this.cobra = [];
    this.gerarComida();
  }

  atualizarDimensoes() {
    this.cols = Math.floor(this.larguraTela() / this.tamanhoQuadrado) || 1;
    this.rows = Math.floor(this.alturaTela() / this.tamanhoQuadrado) || 1;

    // Se a comida estiver fora dos novos limites visíveis, gera uma nova em posição válida
    if (
      this.comida &&
      (this.comida.x >= this.cols || this.comida.y >= this.rows)
    ) {
      this.gerarComida();
    }
  }

  gerarComida() {
    this.comida = new Ponto(
      Math.floor(Math.random() * this.cols),
      Math.floor(Math.random() * this.rows),
    );
  }

  teclaPressionada(e) {
    if (e.code === "ArrowLeft" && this.velocidade.x === 0)
      this.velocidade = new Ponto(-1, 0);
    if (e.code === "ArrowRight" && this.velocidade.x === 0)
      this.velocidade = new Ponto(1, 0);
    if (e.code === "ArrowUp" && this.velocidade.y === 0)
      this.velocidade = new Ponto(0, -1);
    if (e.code === "ArrowDown" && this.velocidade.y === 0)
      this.velocidade = new Ponto(0, 1);

    if (e.altKey && e.code === "KeyF") {
      this.telaCheia();
    }
  }

  atualizar() {
    this.atualizarDimensoes();

    if (this.velocidade.x === 0 && this.velocidade.y === 0) return;

    // Movimentação contínua ajustada às colunas e linhas atuais
    this.posicao.x =
      (this.posicao.x + this.velocidade.x + this.cols) % this.cols;
    this.posicao.y =
      (this.posicao.y + this.velocidade.y + this.rows) % this.rows;

    // Game over ao colidir com o próprio corpo
    for (let p of this.cobra) {
      if (p.x === this.posicao.x && p.y === this.posicao.y) {
        EfeitosSonoros.tocarSom("tiro", false, true);
        this.resetarJogo();
        return;
      }
    }

    this.cobra.unshift(new Ponto(this.posicao.x, this.posicao.y));

    if (this.posicao.x === this.comida.x && this.posicao.y === this.comida.y) {
      EfeitosSonoros.tocarSom("comida", false, true);
      this.tamanhoCobra++;
      this.gerarComida();
    }

    while (this.cobra.length > this.tamanhoCobra) {
      this.cobra.pop();
    }
  }

  desenhar() {
    let sz = this.tamanhoQuadrado;

    this.preenchimento("black");
    this.retangulo(
      0,
      0,
      this.larguraTela(),
      this.alturaTela(),
      Estilo.PREENCHIDO,
    );

    this.preenchimento("blue");
    this.retangulo(
      this.comida.x * sz,
      this.comida.y * sz,
      sz,
      sz,
      Estilo.PREENCHIDO,
    );

    this.preenchimento("greenyellow");
    for (let p of this.cobra) {
      this.retangulo(p.x * sz, p.y * sz, sz - 1, sz - 1, Estilo.PREENCHIDO);
    }
  }
}

window.addEventListener("load", () => {
  new ExemploSnake("Cobrinha", "gameCanvas", 600, 600, 8);
});